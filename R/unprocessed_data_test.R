...
unprocessed_data_test <- function(exp_data, char_table, method = "t.test",
  ...
  Lipid_FA_freq_list <- each_FA %>%
    map(.f = function(x) {
      factor(sort(x[x != "0:0;0"]), levels = FA_mapping) %>%
        table() %>%
        as.integer()
    })
  ...
  FA_exp <- FA_exp %>%
    mutate(feature = FA_mapping, type = "FA") %>%
  ...
  lipid_class_exp <- exp_data %>%
    left_join(char_table[c("feature", "class")], by = "feature") %>%
    dplyr::select(-1)
  ...
  if (method == "mod.t.test") {
    ...
    mod_t_test <- tryCatch(
      {
        mod.t.test(as.matrix(all_exp_data[-c(1:2)]), group = g2)
      },
      error = function(e) {
        return(NULL)
      }
  ...
  for (a in 1:nrow(all_exp_data)) {
    ...
    mean_ctrl[a] <- all_exp_data[a, ctrl_group] %>%
      unlist() %>%
      mean(na.rm = T)
    mean_exp[a] <- all_exp_data[a, exp_group] %>%
      unlist() %>%
      mean(na.rm = T)
    mean_all[a] <- all_exp_data[a, c(ctrl_group, exp_group)] %>%
      unlist() %>%
      mean(na.rm = T)
    
    FC[a] <- mean_exp[a] / mean_ctrl[a]
    sd_ctrl[a] <- all_exp_data[a, ctrl_group] %>%
      unlist() %>%
      sd(na.rm = T)
    sd_exp[a] <- all_exp_data[a, exp_group] %>%
      unlist() %>%
      sd(na.rm = T)
    ...
    if (method == "t.test") {
      p_value[a] <- tryCatch(
        {
          t.test(unlist(all_exp_data[a, exp_group]), unlist(all_exp_data[a, ctrl_group]), var.equal = T)$p.value
        },
        error = function(e) {
          return(NA)
        }
      )
      statistics[a] <- tryCatch(
        {
          t.test(unlist(all_exp_data[a, exp_group]), unlist(all_exp_data[a, ctrl_group]), var.equal = T)$statistic
        },
        error = function(e) {
          return(NA)
        }
      ...
      p_value[a] <- tryCatch(
        {
          wilcox.test(unlist(all_exp_data[a, exp_group]), unlist(all_exp_data[a, ctrl_group]), var.equal = T)$p.value
        },
        error = function(e) {
          return(NA)
        }
      ...
      statistics[a] <- tryCatch(
        {
          wilcox.test(unlist(all_exp_data[a, exp_group]), unlist(all_exp_data[a, ctrl_group]), var.equal = T)$statistic
        },
        error = function(e) {
          return(NA)
        }
  ...
  result <- data.frame(
    lipid = lipid, type = type, mean_all = mean_all, mean_ctrl = mean_ctrl, mean_exp = mean_exp, sd_ctrl = sd_ctrl,
