...
t_test <- function(data, ctrl, exp, method = "t.test", significant = "p_value") {
  ...
  for (a in 1:nrow(data)) {
    ...
    mean_ctrl[a] <- data[a, ctrl] %>%
      unlist() %>%
      mean(na.rm = T)
    mean_exp[a] <- data[a, exp] %>%
      unlist() %>%
      mean(na.rm = T)
    mean_all[a] <- data[a, c(ctrl, exp)] %>%
      unlist() %>%
      mean(na.rm = T)
    FC[a] <- mean_exp[a] / mean_ctrl[a]
    sd_ctrl[a] <- data[a, ctrl] %>%
      unlist() %>%
      sd(na.rm = T)
    sd_exp[a] <- data[a, exp] %>%
      unlist() %>%
    ...
    if (method == "t.test") {
      p_value[a] <- tryCatch(
        {
          t.test(unlist(data[a, exp]), unlist(data[a, ctrl]), var.equal = T)$p.value
        },
        error = function(e) {
          return(NA)
        }
      ...
      statistics[a] <- tryCatch(
        {
          t.test(unlist(data[a, exp]), unlist(data[a, ctrl]), var.equal = T)$statistic
        },
        error = function(e) {
          return(NA)
        }
      ...
      p_value[a] <- tryCatch(
        {
          wilcox.test(unlist(data[a, exp]), unlist(data[a, ctrl]), var.equal = T)$p.value
        },
        error = function(e) {
          return(NA)
        }
      ...
      statistics[a] <- tryCatch(
        {
          wilcox.test(unlist(data[a, exp]), unlist(data[a, ctrl]), var.equal = T)$statistic
        },
        error = function(e) {
          return(NA)
        }
  ...
  result <- data.frame(
    lipid = lipid, mean_all = mean_all, mean_ctrl = mean_ctrl, mean_exp = mean_exp, sd_ctrl = sd_ctrl,
